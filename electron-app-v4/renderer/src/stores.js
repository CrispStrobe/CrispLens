...
const EN = {
  ...
  api_server_hint:            'URL the browser sends all API calls to (images, people, faces, DB). Point this to any CrispLens server — local v4 Node.js, remote v2 FastAPI, or another host. Inference runs on whichever server handles the API.',
  ...
  processing_backend_section: 'Processing Override (local v4 only)',
  processing_backend_hint:    'Only applies when the API server above is the local v4 Node.js. Routes face detection/embedding to a remote server while keeping this v4 as the API/DB server.',
  ...
  settings_db_backup_hint:    'Use these to move your data between browsers or devices.',
  ...
  pv_process_direct_hint:    'Process files now in the browser queue (not persistent)',
  ...
  settings_standalone_active: '⚡ Standalone mode active. All processing, including VLM API calls and user management, happens locally on this device. Ensure you have downloaded the ONNX models below.',
...
export const TRANSLATIONS = {
  ...
  de: {
    ...
    api_server_url_label:       'Server-URL',
    api_server_hint:            'URL, an die der Browser alle API-Aufrufe sendet (Bilder, Personen, Gesichter, DB). Kann auf einen lokalen v4-Node.js, einen Remote-v2-FastAPI oder einen anderen Host zeigen. Die Verarbeitung läuft auf dem Server, der die API bedient.',
    ...
    processing_backend_section: 'Verarbeitungs-Override (nur lokal v4)',
    processing_backend_hint:    'Nur aktiv wenn der API-Server oben der lokale v4-Node.js ist. Leitet Gesichtserkennung/-einbettung an einen Remote-Server weiter, während v4 als API/DB-Server bleibt.',
    ...
    settings_db_backup_hint:    'Nutzen Sie diese Optionen, um Ihre Daten zwischen Browsern oder Geräten zu verschieben.',
    ...
    settings_standalone_active: '⚡ Standalone-Modus aktiv. Alle Verarbeitungsschritte, einschließlich VLM-API-Aufrufe und Benutzerverwaltung, erfolgen lokal auf diesem Gerät. Stellen Sie sicher, dass Sie die unten stehenden ONNX-Modelle heruntergeladen haben.',

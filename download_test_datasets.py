...
def download_att_faces(output_dir="datasets/att_faces"):
    ...
    with zipfile.ZipFile(zip_path, 'r') as zip_ref:
        # Guard against zip-slip: reject members with absolute paths or .. traversal
        out_abs = os.path.abspath(output_dir)
        def _safe_zip(info):
            target = os.path.abspath(os.path.join(output_dir, info.filename))
            if not target.startswith(out_abs + os.sep) and target != out_abs:
                raise RuntimeError(f"Refusing to extract outside {output_dir}: {info.filename}")
            return info.filename
        safe_names = [_safe_zip(m) for m in zip_ref.infolist()]
        zip_ref.extractall(output_dir, members=safe_names)  # nosec B202 — members validated above
...
def quick_setup_att():
    ...
    print("   python face_recognition_benchmark.py \\")
    print("     --mode benchmark \\")
    print(f"     --image {group_dir}/group_1_20people.jpg \\")
    print("     --methods insightface dlib_hog \\")
    print("     --runs 3")
...
def download_lfw_dataset(output_dir="datasets/lfw"):
    ...
    with tarfile.open(tar_path, 'r:gz') as tar:
        # Guard against tar-slip: reject members with absolute paths or .. traversal
        out_abs = os.path.abspath(output_dir)
        def _safe(member):
            target = os.path.abspath(os.path.join(output_dir, member.name))
            if not target.startswith(out_abs + os.sep) and target != out_abs:
                raise RuntimeError(f"Refusing to extract outside {output_dir}: {member.name}")
            return member
        tar.extractall(output_dir, members=(_safe(m) for m in tar))

...
#   CRISP_BACKEND       v2|v4 — backend engine                (default: prompt)
#                         v2 = Python FastAPI (InsightFace / FAISS)
#                         v4 = Node.js Express (onnxruntime-node, same REST API)
...
# ── v4 Node.js systemd service ────────────────────────────────────────────────
create_service_v4() {
    local svc=$1 user=$2 dir=$3 port=$4 adm_user=$5 adm_pass=$6

    local admin_env=""
    [[ -n "$adm_user" ]] && admin_env+="Environment=\"DEFAULT_ADMIN_USER=${adm_user}\"\n"
    [[ -n "$adm_pass" ]] && admin_env+="Environment=\"DEFAULT_ADMIN_PASS=${adm_pass}\"\n"

    {
        printf '[Unit]\n'
        printf 'Description=CrispLens Face Recognition (Node.js v4)\n'
        printf 'After=network-online.target\n'
        printf 'Wants=network-online.target\n'
        printf '\n[Service]\n'
        printf 'Type=simple\n'
        printf 'User=%s\n' "$user"
        printf 'Group=%s\n' "$user"
        printf 'WorkingDirectory=%s/electron-app-v4\n' "$dir"
        printf 'Environment="PATH=/usr/local/bin:/usr/bin:/bin"\n'
        printf 'Environment="PORT=%s"\n' "$port"
        printf 'Environment="DB_PATH=%s/face_recognition.db"\n' "$dir"
        [[ -n "${CRISP_ALLOWED_ORIGINS:-}" ]] && \
            printf 'Environment="CRISP_ALLOWED_ORIGINS=%s"\n' "$CRISP_ALLOWED_ORIGINS"
        [[ -n "$admin_env" ]] && printf '%b' "$admin_env"
        printf 'ExecStart=/usr/bin/node %s/electron-app-v4/server.js\n' "$dir"
        printf 'Restart=on-failure\n'
        printf 'RestartSec=10\n'
        printf 'StandardOutput=journal\n'
        printf 'StandardError=journal\n'
    ...
    systemctl daemon-reload
    systemctl enable "${svc}" --quiet
    info "systemd service '${svc}' (v4 Node.js) registered and enabled"
}

# ── v4 container entrypoint ───────────────────────────────────────────────────
create_entrypoint_v4() {
    local dir=$1
    cat > "${dir}/start.sh" <<'STARTEOF'
#!/usr/bin/env bash
# CrispLens v4 Node.js container entrypoint — generated by deploy-v2.sh
#
# Runtime environment variables (pass with: docker run -e VAR=value ...):
#   PORT                  listen port         (default: 7865)
#   DB_PATH               absolute DB path    (optional, default: /data/face_recognition.db)
#   DEFAULT_ADMIN_USER    bootstrap admin username  (first start only)
#   DEFAULT_ADMIN_PASS    bootstrap admin password  (first start only)
set -e
: "${PORT:=7865}"
...
export PORT DB_PATH
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
exec node "${SCRIPT_DIR}/electron-app-v4/server.js"
STARTEOF
    chmod +x "${dir}/start.sh"
    info "v4 Entrypoint written: ${dir}/start.sh"
}
...
echo
echo -e "${BOLD}${BLUE}╔══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BOLD}${BLUE}║      CrispLens — Deployment Script (v2 FastAPI / v4 Node)    ║${NC}"
echo -e "${BOLD}${BLUE}╚══════════════════════════════════════════════════════════════╝${NC}"
...
# Backend engine choice
if [[ -z "${CRISP_BACKEND:-}" ]]; then
    if [[ "${CRISP_YES:-0}" == "1" ]]; then
        CRISP_BACKEND="v2"
    else
        echo -e "  ${BOLD}Backend engine${NC}:"
        echo -e "    ${GREEN}v2${NC} — Python FastAPI + InsightFace/FAISS  (battle-tested, full feature set)"
        echo -e "    ${GREEN}v4${NC} — Node.js Express + onnxruntime-node  (pure JS, same REST API)"
        read -rp "  Choose backend [v2/v4] (default: v2): " _be
        CRISP_BACKEND="${_be:-v2}"
    fi
fi
[[ "$CRISP_BACKEND" == "v2" || "$CRISP_BACKEND" == "v4" ]] \
    || die "CRISP_BACKEND must be 'v2' or 'v4', got: ${CRISP_BACKEND}"
info "Backend: ${CRISP_BACKEND}"
...
echo -e "  Backend       :  ${CRISP_BACKEND} $([ "$CRISP_BACKEND" == "v4" ] && echo '(Node.js/Express)' || echo '(Python FastAPI)')"
if [[ "$IN_CONTAINER" == true ]]; then
...
if [[ "$CRISP_BACKEND" == "v4" ]]; then
    # v4: Node.js is required at runtime
    RENDERER_DIR="${SCRIPT_DIR}/electron-app-v4/renderer"
    NEED_BUILD=false
    if ! command -v node &>/dev/null; then
        info "Installing Node.js 20 via NodeSource (required for v4 runtime)..."
    ...
    # v2: Node.js only needed for building Svelte frontend
...
# =============================================================================
# PHASE 5 — Python venv + dependencies (v2 only)
# =============================================================================
if [[ "$CRISP_BACKEND" == "v2" ]]; then
...
run_as "${VENV}/bin/pip" install -q imagehash \
    ...
    step "Node.js dependencies (v4)"
    V4_DIR="${CRISP_INSTALL_DIR}/electron-app-v4"
    if [[ -d "$V4_DIR" ]]; then
        run_as bash -c "cd '${V4_DIR}' && npm install --omit=dev --loglevel=error" \
            && info "v4 npm dependencies installed" \
            || warn "npm install reported errors — check output"
    else
        warn "electron-app-v4/ not found in install dir"
    fi
fi
...
if [[ "$CRISP_BACKEND" == "v4" ]]; then
    INSTALL_RENDERER="${CRISP_INSTALL_DIR}/electron-app-v4/renderer"
else
...
if [[ "$NEED_BUILD" == true ]]; then
    if command -v npm &>/dev/null; then
        info "Building Svelte frontend (${CRISP_BACKEND} renderer)..."
        run_as bash -c "
            cd '${INSTALL_RENDERER}'
...
# =============================================================================
# PHASE 7 — config.yaml (v2 only; v4 uses DB settings table + env vars)
# =============================================================================
if [[ "$CRISP_BACKEND" == "v2" ]]; then
...
if [[ -f "$CFG" ]]; then
    ...
    info "v4 backend — config.yaml not used (settings stored in DB)"
fi
...
if [[ "$IN_CONTAINER" == false ]]; then
    ...
    if [[ "$CRISP_BACKEND" == "v4" ]]; then
        create_entrypoint_v4 "${CRISP_INSTALL_DIR}"
    else
...
if [[ "$IN_CONTAINER" == true ]]; then
    ...
    echo
    echo -e "  ${DIM}Update after git pull:${NC}"
    if [[ "$CRISP_BACKEND" == "v4" ]]; then
        echo -e "    cd ${CRISP_INSTALL_DIR} && sudo -u ${CRISP_SVC_USER} git pull"
        echo -e "    cd electron-app-v4 && sudo -u ${CRISP_SVC_USER} npm install --omit=dev"
        echo -e "    cd renderer && sudo -u ${CRISP_SVC_USER} npm install && npm run build && cd ../.."
        echo -e "    systemctl restart ${CRISP_SVC_NAME}"
    else
        echo -e "    cd ${CRISP_INSTALL_DIR} && sudo -u ${CRISP_SVC_USER} git pull"
